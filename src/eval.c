 ...
 *
 * astFree is not called in evalAST for each node as there are parent owns children,
 *      so could result in double frees. Caller frees the AST nodes after evaluation.
 * 0 is usd as an error-sentinel, but needs to be fixed somehow
 *      (maybe by changing the return type)
 * Assignment nodes are dealt with separately from other binary operation nodes,
 *      as trigger manipulation of the symbol table itself
 * Increment and Decrement is handled separately from other assignment operations, 
