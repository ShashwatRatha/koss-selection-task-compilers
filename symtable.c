...
static SymResult symUpdate(SymTable* t, int i, int op, int rval) {
    ...
    switch (op) {
        case ASSGN:        val  = rval;  
                           break;
        case ADD_ASSGN:    val += rval;  
                           break;
        case SUB_ASSGN:    val -= rval;  
                           break;
        case MUL_ASSGN:    val *= rval;  
                           break;
        case DIV_ASSGN:    if (rval == 0)
                               return SYM_DIVZERO;
                           val /= rval;
                           break;
        case LSHIFT_ASSGN: val <<= rval; 
                           break;
        case RSHIFT_ASSGN: val >>= rval; 
                           break;
        default:           return SYM_ERR_UNDEF;
    }
    t->entries[i].val = val;
    return SYM_OK;
}
